...
import { createContext, useContext, useEffect, useOptimistic, useReducer, useRef, type ReactNode } from "react";
import type { Product, Variant } from "@/lib/types";

export type CartItem = { uid: string; name: string; image?: string; variant: Variant; quantity: number };
type Action = { type: "add"; item: CartItem } | { type: "remove"; key: string } | { type: "update"; key: string; quantity: number } | { type: "clear" };
...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    try {
      const stored = localStorage.getItem("walton-cart");
      if (stored) {
        const parsed = JSON.parse(stored) as CartItem[];
        parsed.filter((item) => item?.uid && item?.variant).forEach((item) => dispatch({ type: "add", item }));
      }
    } catch { /* storage is optional */ }
  }, []);
  useEffect(() => { if (hydrated.current) localStorage.setItem("walton-cart", JSON.stringify(items)); }, [items]);
  useEffect(() => { hydrated.current = true; }, []);
  const add = (product: Product, variant: Variant) => { const item = { uid: product.uid, name: product.enName ?? "Walton product", image: product.images?.[0]?.url ?? undefined, variant, quantity: 1 }; setOptimisticItems({ type: "add", item }); dispatch({ type: "add", item }); };
  const value = { items: optimisticItems, count: optimisticItems.reduce((sum, item) => sum + item.quantity, 0), add, remove: (key: string) => dispatch({ type: "remove", key }), update: (key: string, quantity: number) => dispatch({ type: "update", key, quantity }), clear: () => dispatch({ type: "clear" }) };
