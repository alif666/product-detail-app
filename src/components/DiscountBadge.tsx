...
export function DiscountBadge({discount, compact = false}: { discount: DiscountInfo; compact?: boolean }) {
    ...
    if (compact) {
        return <span className="inline-flex flex-col items-center justify-center rounded-full bg-gradient-to-r from-[#b91c1c] via-[#991b1b] to-[#7f1d1d] px-3 py-1 text-white shadow-sm"><strong className="text-sm leading-4">{value}</strong><span className="text-[9px] font-bold uppercase leading-3">{discount.type === "flat" ? "Save" : "OFF"}</span></span>;
    }
