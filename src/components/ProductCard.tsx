...
export function ProductCard({product}: { product: Product }) {
    ...
    return <article
        ...
        <Link href={`/products/${product.uid}`} className="block">
            <div className="relative aspect-[4/3] overflow-hidden bg-slate-100">{image ?
                ...
                className="absolute left-3 top-3 rounded-full bg-[#ec1c24] px-2.5 py-1 text-xs font-bold text-white">{Math.round(discount)}% OFF</span> : null}</div>
            <div className="p-4"><p
                className="mb-2 line-clamp-2 min-h-10 text-sm font-bold leading-5 text-slate-800">{product.enName ?? "Walton product"}</p>
                <div className="flex items-baseline gap-2"><strong
                    className="text-lg text-[#ec1c24]">{formatPrice(price)}</strong>{oldPrice > price ?
                ...
                {variant?.quantity === 0 ? <p className="mt-2 text-xs font-semibold text-red-600">Out of stock</p> :
                    <p className="mt-2 text-xs font-medium text-emerald-600">In stock</p>}</div>
        </Link></article>;
}
