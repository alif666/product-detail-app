...
import {ShoppingCartIcon, SparklesIcon, StoreIcon, WaltonLogo} from "@/components/Icons";

export function Header() {
    const {count} = useCart();
    const [cartOpen, setCartOpen] = useState(false);
    return <>
        <header className="sticky top-0 z-20 border-b border-slate-200 bg-[#ddf0f4] backdrop-blur">
        <div className="mx-auto flex h-[76px] max-w-7xl items-center justify-between gap-6 px-5 lg:px-8"><Link href="/"
                                                                                                     aria-label="Walton Plaza home"
                                                                                                     className="flex shrink-0 items-center"><WaltonLogo className="h-auto w-[170px] sm:w-[216px]"/></Link>
