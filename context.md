...
## Latest implementation update: listing performance improvements

- Added a granular Suspense boundary in `src/app/page.tsx`. The header, hero, and listing heading render as the static shell while the API-dependent listing section streams behind `ProductSectionSkeleton`. Existing URL-driven UID search, page size, pagination, filters, sorting, errors, and product links remain supported.
- Added `ProductSectionSkeleton` to `src/components/LoadingSkeleton.tsx` for a section-sized fallback. The existing route-level listing and detail skeletons remain unchanged.
- Split GraphQL field selection in `src/lib/queries.ts`: `PRODUCTS_QUERY` now uses the lightweight `ProductCardFields` selection (`uid`, `enName`, `images`, `productAttributes`, and variant pricing/stock/discount identifiers), while `PRODUCT_QUERY` retains the full detail fields.
